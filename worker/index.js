...
async function api(request, env) {
  ...
  if (url.pathname === "/api/register" && request.method === "POST") {
    ...
    await env.DB.prepare("INSERT INTO users(user_id,username,password_hash,mobile,email,balance,created_at) VALUES(?,?,?,?,?,?,?)")
      .bind(userId, username, passwordHash, mobile, email, 0, now()).run();
